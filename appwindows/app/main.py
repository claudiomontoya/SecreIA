...
def format_date_chile(date_str: str) -> str:
    """Formatea fecha para Windows con información local"""
    try:
        # Parsear la fecha
        ...
        if diff.days == 0:
            ...
            return f"{dt_local.day:02d}/{dt_local.month:02d} {dt_local.strftime('%H:%M')}"
        elif dt_local.year == now_local.year:
            month_short = months_es[dt_local.month - 1]
            return f"{dt_local.day} {month_short} {dt_local.strftime('%H:%M')}"
        else:
...
class WindowsColors:
    # Backgrounds - Estilo Windows 11
    SIDEBAR = QColor(243, 243, 243)           
    NOTES_LIST = QColor(255, 255, 255)       
    CONTENT = QColor(255, 255, 255)          
    CARD = QColor(249, 249, 249)             
    ELEVATED = QColor(245, 245, 245)         
    
    # Text
    PRIMARY = QColor(32, 31, 30)       
    SECONDARY = QColor(96, 94, 92)     
    TERTIARY = QColor(161, 159, 157)        
    
    # Accents Windows 11
    BLUE = QColor(0, 120, 215)          
    GREEN = QColor(16, 137, 62)          
    RED = QColor(196, 43, 28)            
    ORANGE = QColor(247, 130, 0)        
    PURPLE = QColor(135, 100, 184)        
    
    # Separators
    SEPARATOR = QColor(225, 223, 221)        
    SEPARATOR_LIGHT = QColor(237, 235, 233)

if platform.system() == "Windows":
    AppColors = WindowsColors
...
class LoadingSpinner(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(8):
            alpha = 255 - (i * 30)
            painter.setPen(QPen(QColor(WindowsColors.SECONDARY.red(), 
                                     WindowsColors.SECONDARY.green(), 
                                     WindowsColors.SECONDARY.blue(), alpha), 2))
...
class StatusBadge(QLabel):
    ...
    def set_status(self, status: str):
        colors = {
            "saved": "transparent",
            "saving": WindowsColors.ORANGE.name(),
            "unsaved": WindowsColors.RED.name(),
            "syncing": WindowsColors.BLUE.name()
        }
...
class SummaryTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        header.setStyleSheet(f"""
            QLabel {{
                color: {WindowsColors.PRIMARY.name()};
        ...
        description.setStyleSheet(f"""
            QLabel {{
                color: {WindowsColors.SECONDARY.name()};
                font-family: '.AppleSystemUIFont';
                font-size: 16px;
            }}
        """)
        ...
        self.summary_text.setStyleSheet(f"""
            QTextEdit {{
                background-color: {WindowsColors.NOTES_LIST.name()};
                color: {WindowsColors.PRIMARY.name()};
                border: none;
                border-radius: 12px;
                padding: 20px;
    ...
    def _play_audio(self):
        ...
        try:
            ...
            self.btn_play_audio.setStyleSheet(f"""
                QPushButton {{
                    background-color: {WindowsColors.RED.name()};
                    color: white;
                    border: none;
                    border-radius: 8px;
                    padding: 10px 20px;
                ...
                QPushButton:hover {{
                    background-color: {WindowsColors.RED.darker(110).name()};
                }}
            """)
    ...
    def _reset_audio_button(self):
        ...
        self.btn_play_audio.setStyleSheet(f"""
            QPushButton {{
                background-color: {WindowsColors.CARD.name()};
                color: {WindowsColors.PRIMARY.name()};
                border: 1px solid {WindowsColors.SEPARATOR_LIGHT.name()};
                border-radius: 8px;
                padding: 10px 20px;
                font-family: '.AppleSystemUIFont';
                font-size: 14px;
            ...
            QPushButton:hover {{
                background-color: {WindowsColors.ELEVATED.name()};
            }}
        """)
    ...
    def _setup_ui(self):
        ...
        self.search_edit.setStyleSheet(f"""
            QLineEdit {{
                ...
                font-family: '.AppleSystemUIFont';
                font-size: 14px;
                selection-background-color: {WindowsColors.BLUE.name()};
            }}
            QLineEdit:focus {{
                border: 2px solid {WindowsColors.BLUE.name()};
                padding: 11px 15px;
            }}
        """)
...
class AudioQualityWidget(QWidget):
    ...
    def update_quality(self, volume: float, quality: str):
        ...
        color_map = {
            "good": WindowsColors.GREEN.name(),
            "low": WindowsColors.ORANGE.name(),
            "silent": WindowsColors.TERTIARY.name(),
            "clipping": WindowsColors.RED.name()
        }
        
        color = color_map.get(quality, WindowsColors.SECONDARY.name())
...
class AppleButton(QPushButton):
    ...
    def _setup_style(self):
        styles = {
            "primary": f"""
                QPushButton {{
                    background-color: {WindowsColors.BLUE.name()};
                    color: white;
                    border: none;
                    border-radius: 8px;
                    padding: 10px 20px;
                ...
                QPushButton:hover {{
                    background-color: {WindowsColors.BLUE.lighter(110).name()};
                }}
                QPushButton:disabled {{
                    background-color: {WindowsColors.TERTIARY.name()};
                }}
            """,
            "secondary": f"""
                QPushButton {{
            ...
            "success": f"""
                ...
                QPushButton:hover {{
                    background-color: {WindowsColors.GREEN.darker(110).name()};
                }}
                QPushButton:disabled {{
                    background-color: {WindowsColors.TERTIARY.name()};
                }}
            ...
            "danger": f"""
                QPushButton {{
                    background-color: transparent;
                    color: {WindowsColors.RED.name()};
                    border: 1px solid {WindowsColors.RED.name()};
                    border-radius: 8px;
                    padding: 10px 20px;
                    font-family: '.AppleSystemUIFont';
                    font-size: 14px;
...
class NotesListDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # Fondo
        if is_selected:
            painter.fillRect(rect, WindowsColors.BLUE)
            text_color = QColor(255, 255, 255)
            meta_color = QColor(255, 255, 255, 180)
        else:
            text_color = WindowsColors.PRIMARY
            meta_color = WindowsColors.SECONDARY
        ...
        if has_audio or is_transcript:
            ...
            if has_audio:
                painter.setPen(QPen(WindowsColors.BLUE))
                painter.setFont(QFont(".AppleSystemUIFont", 12))
                painter.drawText(icon_x, icon_y + 15, "🎵")
                
            if is_transcript:
                painter.setPen(QPen(WindowsColors.GREEN))
                painter.setFont(QFont(".AppleSystemUIFont", 12))
...
class EnhancedNoteEditor(QWidget):
    ...
    def _show_success_state(self):
            ...
            self.save_status.setStyleSheet(f"""
                QLabel {{
                    color: {WindowsColors.GREEN.name()};
                    font-size: 12px;
                    padding: 4px 8px;
                }}
            """)
...
class EnhancedNotesView(QWidget):
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        menu.setStyleSheet(f"""
            QMenu {{
                background-color: {WindowsColors.ELEVATED.name()};
                border: 1px solid {WindowsColors.SEPARATOR.name()};
                border-radius: 8px;
                padding: 4px 0;
                font-size: 14px;
                color: {WindowsColors.PRIMARY.name()};
...
class EnhancedTranscribeTab(QWidget):
    ...
    def _show_permission_guide(self):
        """Muestra guía detallada para habilitar permisos en Windows"""
        guide_text = """
    Para habilitar el micrófono en Windows:

    1. Presiona tecla Windows + I para abrir Configuración
    2. Ve a "Privacidad y seguridad"
    3. En el panel izquierdo, busca y selecciona "Micrófono"
    ...
    5. Si SecreIA aparece en la lista de aplicaciones, asegúrate de que esté activada

    Después de cambiar los permisos, reinicia SecreIA.

    NOTA: Si Windows Defender bloquea la aplicación, agrégala como excepción en 
    ...
    def _open_system_preferences(self):
            ...
            try:
                # Abrir configuración de privacidad de micrófono en Windows
                subprocess.run(["ms-settings:privacy-microphone"], shell=True)
            except Exception as e:
                try:
                    # Fallback: abrir configuración general de Windows
                    subprocess.run(["ms-settings:"], shell=True)
                except Exception:
                    # Último fallback: mostrar instrucciones manuales
                    QMessageBox.information(
                        self,
                        "Configurar permisos manualmente",
                        "Para permitir acceso al micrófono:\n\n"
                        "1. Ve a Configuración (tecla Windows + I)\n"
                        "2. Selecciona 'Privacidad y seguridad'\n"
                        "3. En el panel izquierdo, busca 'Micrófono'\n"
                        "4. Activa 'Permitir acceso al micrófono'\n"
                        "5. Activa 'Permitir que las aplicaciones de escritorio accedan al micrófono'\n"
                        "6. Reinicia SecreIA después de cambiar los permisos"
                    )

    def _request_microphone_permission(self):
        """Solicita permisos de micrófono específico para Windows"""
        try:
            reply = QMessageBox.question(
                self, 
                "Permisos de micrófono",
                "Esta aplicación necesita acceso al micrófono para transcribir audio.\n\n"
                "Windows requiere que permitas el acceso manualmente.\n"
            ...
            QMessageBox.critical(
                self,
                "Error de permisos",
                f"No se pudieron configurar permisos de micrófono.\n\n"
                f"Error: {e}\n\n"
                f"Ve a Configuración > Privacidad y seguridad > Micrófono\n"
                f"y activa el acceso para aplicaciones de escritorio."
            )
            return False
    ...
    def _setup_ui(self):
        ...
        separator.setStyleSheet(f"""
            QFrame {{
                color: {WindowsColors.SEPARATOR_LIGHT.name()};
                margin: 8px 0;
            }}
        """)
        ...
        self.title_edit.setStyleSheet(f"""
            QLineEdit {{
                background-color: {WindowsColors.SIDEBAR.name()};
                color: {WindowsColors.PRIMARY.name()};
                border: 1px solid {WindowsColors.SEPARATOR.name()};
                border-radius: 6px;
                padding: 8px 12px;
...
class CategoriesTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.categories_list.setStyleSheet(f"""
            ...
            QListWidget::item:hover {{
                background-color: {WindowsColors.ELEVATED.name()};
                border-color: {WindowsColors.BLUE.name()};
            }}
            QListWidget::item:selected {{
                background-color: {WindowsColors.BLUE.name()};
                color: white;
...
class MainWindow(QMainWindow):
    ...
    def _setup_style(self):
        self.setStyleSheet(f"""
            QMainWindow {{
                background-color: {WindowsColors.CONTENT.name()};
            }}
        """)
    ...
    def _setup_main_app(self):
        ...
        self.nav.setStyleSheet(f"""
            QWidget {{
                background-color: {WindowsColors.SIDEBAR.name()};
                border-right: 1px solid {WindowsColors.SEPARATOR.name()};
            }}
        """)
...
def main():
    ...
    # Configurar fuente del sistema para Windows
    font = QFont("Segoe UI", 11)
    app.setFont(font)
    
    # Paleta de colores Windows
    palette = QPalette()
    palette.setColor(QPalette.Window, WindowsColors.CONTENT)
    palette.setColor(QPalette.WindowText, WindowsColors.PRIMARY)
    palette.setColor(QPalette.Base, WindowsColors.NOTES_LIST)
    palette.setColor(QPalette.AlternateBase, WindowsColors.CARD)
    palette.setColor(QPalette.ToolTipBase, WindowsColors.ELEVATED)
    palette.setColor(QPalette.ToolTipText, WindowsColors.PRIMARY)
    palette.setColor(QPalette.Text, WindowsColors.PRIMARY)
    palette.setColor(QPalette.Button, WindowsColors.CARD)
    palette.setColor(QPalette.ButtonText, WindowsColors.PRIMARY)
    palette.setColor(QPalette.Highlight, WindowsColors.BLUE)
    palette.setColor(QPalette.HighlightedText, QColor(255, 255, 255))
    palette.setColor(QPalette.Link, WindowsColors.BLUE)
    
    # Disabled states
    palette.setColor(QPalette.Disabled, QPalette.Text, WindowsColors.TERTIARY)
    palette.setColor(QPalette.Disabled, QPalette.WindowText, WindowsColors.TERTIARY)
    palette.setColor(QPalette.Disabled, QPalette.ButtonText, WindowsColors.TERTIARY)
    palette.setColor(QPalette.Disabled, QPalette.Highlight, WindowsColors.SEPARATOR)
    palette.setColor(QPalette.Disabled, QPalette.HighlightedText, WindowsColors.PRIMARY)
    
    app.setPalette(palette)
    
    # Tooltips estilo Windows
    app.setStyleSheet(f"""
        QToolTip {{
            background-color: {WindowsColors.ELEVATED.name()};
            color: {WindowsColors.PRIMARY.name()};
    ...
    try:
        ...
        # Manejo de señales del sistema para Windows
        import signal
